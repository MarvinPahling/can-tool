...
- `src/lib/connect-settings.ts` + `src/hooks/use-connect-settings.ts` — the same store pattern for device connection settings (`can-tool:connect-settings`); currently just `readOnly`. The selected port and bitrate are deliberately still session-only.
