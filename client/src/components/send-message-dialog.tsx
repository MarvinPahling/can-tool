...
import { useCommandHandler, popScope, pushScope } from "@/commands";
import { useCurrentDbc } from "@/queries/dbc";
import { useConnectionStatus, useGenerateChecksum, useSendCanMessage } from "@/queries/can";
...
export function SendMessageDialog() {
  ...
  >({});
  const [checksumSignals, setChecksumSignals] = useState<Record<number, string>>({});
  ...
  useEffect(() => {
    if (!pending.data || pending.data.nonce === lastHandledNonce.current) return;
    lastHandledNonce.current = pending.data.nonce;

    setOpen(true);
    const frames = form.state.values.frames;
    const existingIndex = frames.findIndex((frame) => frame.messageId === pending.data!.messageId);
    if (existingIndex === -1) {
      form.insertFieldValue("frames", 0, { messageId: pending.data.messageId, values: {} });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Compose frames from the loaded DBC's message definitions and transmit them.
          </DialogDescription>
        </DialogHeader>
        ...
        {!dbc.data ? (
          <Alert>
            ...
            <AlertDescription>Open a .dbc file before composing messages.</AlertDescription>
          </Alert>
        ) : !status.data ? (
          <Alert>
            <AlertTitle>No device connected</AlertTitle>
            <AlertDescription>
              Connect a CAN adapter before sending. You can still compose and validate frames.
            </AlertDescription>
          </Alert>
        ) : null}
        ...
        {sendMessage.isError && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              {sendMessage.error instanceof Error ? sendMessage.error.message : "Unknown error"}
            </AlertDescription>
          </Alert>
        )}
                ...
                return (
                  <FrameRow
                    ...
                    onChecksumSignalChange={(signalName) =>
                      setChecksumSignals((prev) => ({ ...prev, [id]: signalName }))
                    }
                    canSend={Boolean(status.data)}
                    onSend={(message, values) => sendMessage.mutate({ message, values })}
                    onRemove={() => {
...
function FrameRow({
  ...
  const frame = useStore(form.store, (state: { values: SendFormValues }) => state.values.frames[index]);
  const selectedMessage = messages.find((m) => String(m.id) === frame?.messageId);
    ...
    <div className="space-y-2 rounded-md border border-border p-3">
      <div className="flex items-center gap-2">
        <form.Field
          ...
          validators={{
            onChange: ({ value }: { value: string }) => (value ? undefined : "Select a message"),
          }}
        >
          {(field: any) => (
            <Select
            ...
            >
              <SelectTrigger className="w-56" aria-invalid={field.state.meta.errors.length > 0}>
                <SelectValue placeholder="Select a message">
                  {(value: string | null) =>
                    messages.find((message) => String(message.id) === value)?.name ??
                    "Select a message"
                  }
                </SelectValue>
              </SelectTrigger>
        ...
        {selectedMessage && (
          <span className="text-xs text-muted-foreground">
            ID 0x{selectedMessage.id.toString(16).toUpperCase()} · {selectedMessage.size} bytes
          </span>
        )}
        ...
        <>
            ...
            {selectedMessage.signals.map((signal) => {
              ...
              <form.Field
                ...
                validators={{
                  ...
                  onChangeAsync: async () => {
                    const currentValues = form.getFieldValue(`frames[${index}].values`) as Record<
                      string,
                      number
                    >;
                    try {
                      const bytes = await encodeCanMessage(selectedMessage, currentValues);
              ...
              >
                {(field: any) => {
                  ...
                  const errorId =
                    field.state.meta.errors.length > 0 ? `${signal.name}-error-${index}` : undefined;
                  ...
                  return isBoolean ? (
                    <label className="flex items-center justify-between gap-2 text-xs">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked: boolean) => field.handleChange(checked ? 1 : 0)}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <SelectTrigger className="w-48">
                <SelectValue placeholder="Checksum field">
                  {(value: string | null) =>
                    selectedMessage.signals.find((s) => s.name === value)?.name ??
                    "Checksum field"
                  }
                </SelectValue>
              </SelectTrigger>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                form.setFieldValue(`frames[${index}].values.${checksumSignal}`, result);
              }}
            >
              Generate
            </Button>
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <Button
              ...
              onClick={() => selectedMessage && onSend(selectedMessage, frame.values)}
            >
              Send
            </Button>
          </div>
