  ...
  return (
    <div className="flex flex-col gap-1.5 py-2.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Badge variant="outline" className={cn("font-mono", isRecording && "animate-pulse")}>
          {isRecording ? "Press keys…" : formatBinding(binding)}
        </Badge>
        {customizable ? (
          <>
            <Button size="sm" variant="ghost" onClick={onStartRecording} disabled={isRecording}>
              Change
            </Button>
            <Button size="sm" variant="ghost" onClick={() => resetCommandBinding(id)}>
              Reset
            </Button>
          </>
        ) : (
    ...
    >
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Click "Change" and press a new key combination to customize a shortcut.
          </DialogDescription>
        </DialogHeader>
