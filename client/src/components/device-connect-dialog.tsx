...
export function DeviceConnectDialog() {
  ...
  useEffect(() => {
    if (!selectedPort && devices.data && devices.data.length > 0) {
      const preferred = devices.data.find((d) => d.is_canable) ?? devices.data[0];
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        {status.data && (
          <Alert>
            ...
            <AlertDescription>
              {status.data.port_name} @ {status.data.bitrate.toLocaleString()} bit/s
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <RefreshCw className={cn("size-4", devices.isFetching && "animate-spin")} />
            Refresh
          </Button>
        </div>
            ...
            devices.data.map((device) => (
              ...
              >
                <span className="min-w-0">
                  <span className="block truncate font-medium">{device.port_name}</span>
                  {device.manufacturer && (
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <SelectTrigger className="w-40">
              <SelectValue placeholder="Bitrate">
                {(value: string | null) =>
                  BITRATES.find((option) => String(option.value) === value)?.label ?? "Bitrate"
                }
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
          ...
          {isConnected ? (
            ...
            <Button
              onClick={() => selectedPort && connect.mutate({ portName: selectedPort, bitrate })}
