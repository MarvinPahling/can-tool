...
export const dbcColumns = columnHelper.columns([
  ...
  columnHelper.accessor(
    (row) => (row.kind === "message" ? row.message.id : row.signal.start_bit),
    {
      ...
      cell: (info) => {
        ...
        return row.kind === "message" ? toHex(row.message.id) : row.signal.start_bit;
      },
  ...
  columnHelper.accessor(
    (row) => (row.kind === "message" ? row.message.size : row.signal.size),
    {
      ...
      cell: (info) => {
        const row = info.row.original;
        return row.kind === "message" ? `${row.message.size} B` : `${row.signal.size} bit`;
      },
      enableGlobalFilter: false,
    },
  ),
  columnHelper.display({
    ...
    cell: (info) => {
      ...
      const endianness = row.signal.little_endian ? "little-endian" : "big-endian";
  ...
  columnHelper.accessor((row) => (row.kind === "signal" ? row.signal.unit : ""), {
    id: "unit",
    header: "Unit",
    enableGlobalFilter: false,
  }),
