...
export const bindingOverridesStore = new Store<BindingOverrides>(
	loadOverrides(),
);
...
bindingOverridesStore.subscribe(() => {
	...
	localStorage.setItem(
		STORAGE_KEY,
		JSON.stringify(bindingOverridesStore.state),
	);
});
...
export function useEffectiveBinding(commandId: string): CommandBinding {
	return useStore(
		...
		(state) => state[commandId] ?? COMMANDS_BY_ID[commandId]!.defaultBinding,
	);
}
